    ...
    def printBitMapForShiftingReg(self):
        if self.debug:
            ...
            for row in self.bitMapSReg:
                ...
                for pixel in row:
                    if pixel != 0:
                        self.offset_canvas.SetPixel(2*pixlIdx+pixlIdx,      2*rowIdx+rowIdx,255,255,255)
                        self.offset_canvas.SetPixel(2*pixlIdx+pixlIdx+1,    2*rowIdx+rowIdx,255,255,255)
                        self.offset_canvas.SetPixel(2*pixlIdx+pixlIdx,      2*rowIdx+rowIdx+1,255,255,255)
